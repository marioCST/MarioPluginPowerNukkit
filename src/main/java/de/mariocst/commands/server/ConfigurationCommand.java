...
public class ConfigurationCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String s, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("mario.configuration") || player.hasPermission("mario.*") || player.hasPermission("*") || player.isOp()) {
                try {
                    if (args.length == 1) {
            ...
            try {
                if (args.length == 1) {
                    switch (args[0].toLowerCase()) {
                        ...
                        default -> sender.sendMessage(MarioMain.getPrefix() + "/config <reload|save>");
                    }
                }
                else {

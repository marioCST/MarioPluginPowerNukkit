            ...
            if (player.hasPermission("mario.nick") || player.hasPermission("mario.*") || player.hasPermission("*") || player.isOp()) {
                if (args.length == 1) {
                    if (args[0].length() < 16) {
                        ...
                        player.sendMessage(MarioMain.getPrefix() + "Dein Nickname wurde erfolgreich zu " + newNick + " geändert!");
                    }
                    else {
                        player.sendMessage(MarioMain.getPrefix() + "Bitte wähle einen Namen, der kürzer als 16 Zeichen ist!");
                        player.getLevel().addSound(player.getLocation(), Sound.RANDOM_ANVIL_LAND);
                    }
                }
                else {
                    player.sendMessage(MarioMain.getPrefix() + "Bitte gib einen Nickname ein, oder suche dir einen, der keine Leerzeichen hat!");
                    player.getLevel().addSound(player.getLocation(), Sound.RANDOM_ANVIL_LAND);
                }
            }
            else {
